...
4.  **设计选项与自定义判断**：为每个原子问题，设计选项并决定是否允许用户自定义输入。
    * **a. 设计核心选项**：设计2-4个清晰且有代表性的选项。这些选项应覆盖利益相关者最可能考虑的几种情况。
    * **b. 判断是否允许自定义**：在设计完核心选项后，判断这个问题是否适合让用户输入自定义答案。如果提供的选项可能无法完全覆盖所有合理场景，或问题本身有开放性，则应将 `allow_custom` 设置为 `true`。反之，则设置为 `false`。
    * **c. 严禁生成通用选项**：你不能在 `options` 数组中直接创建文本为“其他”、“自定义”、“以上都不是”或任何含义相近的通用选项。设置 `allow_custom: true` 的目的就是为了标记该问题允许用户提供选项之外的自定义答案。
...
* 每个"问题对象"必须包含：
    ...
    * `allow_custom` (布尔值): **是否允许用户提供选项之外的自定义答案。如果为 `true`，就不应在 `options` 创建“其他”这样的选项。**
* 每个"选项对象"必须包含：
